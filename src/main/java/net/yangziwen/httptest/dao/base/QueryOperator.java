...
public enum QueryOperator {
	...
	contain {
		@Override
		public OperationParsedResult buildResult(String fieldName, String originKey, String placeholderPrefix, String placeholderSuffix) {
			String placeholder = concatFunction().render("'%'", wrapPlaceholder(originKey, placeholderPrefix, placeholderSuffix), "'%'");
	...
	private static VarArgsSQLFunc concatFunction = null;
	
	private static VarArgsSQLFunc concatFunction() {
		if(concatFunction != null) {
			return concatFunction;
		}
		synchronized(QueryOperator.class) {
			if(concatFunction == null) {
				concatFunction = VarArgsSQLFunc.getConcatFunc(SpringUtil.getBean(DataSource.class));
			}
		}
		return concatFunction;
	}
